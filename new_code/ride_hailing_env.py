...
import generate_simulator as simulator_module


class _SimulatorCompatUnpickler(pickle.Unpickler):
    LEGACY_CLASS_MAP = {
        ('__main__', 'PassengerSimulator'): simulator_module.PassengerSimulator,
        ('__mp_main__', 'PassengerSimulator'): simulator_module.PassengerSimulator,
        ('__main__', 'HexGridProcessor'): simulator_module.HexGridProcessor,
        ('__mp_main__', 'HexGridProcessor'): simulator_module.HexGridProcessor,
    }

    def find_class(self, module, name):
        mapped = self.LEGACY_CLASS_MAP.get((module, name))
        if mapped is not None:
            return mapped
        return super().find_class(module, name)


def _load_simulator_pickle(simulator_path):
    with open(simulator_path, 'rb') as f:
        return _SimulatorCompatUnpickler(f).load()


class RideHailingEnv:
    def __init__(self, simulator_path, fixed_drivers=True, fixed_sim=True):
        self.time = 0
        self.simulator = _load_simulator_pickle(simulator_path)
        ...
        for h_id, neighbors in self.simulator.adjacency.items():
            ...
            self.adjacency_indices[idx] = {d: self.hex_to_idx[n_h] for d, n_h in neighbors.items()}
        ...
        self.served_wait_time = 0.0

    def _next_reset_seed(self):
        self.reset_counter += 1
        return 42 + self.reset_counter

    def reset(self):
        self.time = 0
        seed = self._next_reset_seed()
        if not self.fixed_sim:
            self.simulator.reset_fixed_orders(seed)

        driver_seed = 42 if self.fixed_drivers else seed
        rng = np.random.RandomState(driver_seed)
        self.driver_locations = rng.randint(0, self.n_zones, size=CONFIG['N_DRIVERS'])
        self.driver_status = np.zeros(CONFIG['N_DRIVERS'], dtype=int)  # 0=idle,1=busy,2=offline
        self.driver_free_time = np.zeros(CONFIG['N_DRIVERS'], dtype=int)
        self.driver_daily_income = np.zeros(CONFIG['N_DRIVERS'], dtype=float)
        self.driver_active_steps = np.zeros(CONFIG['N_DRIVERS'], dtype=float)
    ...
    def _get_state_and_mask(self, surge, subsidy):
        ...
        self.cached_grid_features = grid_features
    ...
    def compile(self, platform_params):
        ...
        t = self.time / CONFIG['TIME_STEPS_PER_DAY']
        surge = platform_params['surge'](t, order_counts, idle_driver_counts, sd)
        subsidy = platform_params['subsidy'](t, order_counts, idle_driver_counts, sd)
    ...
    @staticmethod
    def _trip_steps(order):
        return max(1, int(order.get('duration', 1)))

    @staticmethod
    def _trip_distance_km(order):
        return max(0.1, float(order.get('distance', 0.0)) * CONFIG.get('DISTANCE_TO_KM', 1.0))

    def _trip_fare(self, order, surge_multiplier):
        distance_km = self._trip_distance_km(order)
        base_fare = CONFIG['BASE_FARE'] + CONFIG['PRICE_PER_KM'] * distance_km
        return float(surge_multiplier * base_fare)
    ...
    def step(self, actions, platform_params, value_estimator=None):
        ...
        step_revenue, step_subsidy_cost = 0.0, 0.0
        ...
        for loc, d_list in zone_intent_drivers.items():
            ...
            if not local_orders:
                for d_id in d_list:
                    rewards[d_id] = CONFIG['IDLE_REWARD']
                continue

            n_drivers, n_orders = len(d_list), len(local_orders)
            cost_matrix = np.zeros((n_drivers, n_orders))
            if value_estimator is not None:
                query_d_ids = [d_id for d_id in d_list for _ in local_orders]
                query_dest_idxs = [o['dest_idx'] for _ in d_list for o in local_orders]
                future_vals_matrix = value_estimator(query_d_ids, query_dest_idxs).reshape(n_drivers, n_orders)
            else:
                future_vals_matrix = np.zeros((n_drivers, n_orders))

            for i, d_id in enumerate(d_list):
                for j, o in enumerate(local_orders):
                    trip_steps = self._trip_steps(o)
                    price = self._trip_fare(o, surge[loc])
                    fuel = trip_steps * DRIVER_CONFIG['FUEL_COST_PER_STEP']
                    immediate_reward = (1 - commission) * price - fuel + subsidy[loc]
                    cost_matrix[i, j] = -(immediate_reward + 0.99 * future_vals_matrix[i, j])
            ...
            for i, j in zip(row_ind, col_ind):
                ...
                self.served_wait_time += order['wait_time']

                trip_steps = self._trip_steps(order)
                price = self._trip_fare(order, surge[loc])
                driver_income = (1 - commission) * price - trip_steps * DRIVER_CONFIG['FUEL_COST_PER_STEP'] + subsidy[loc]
                actual_income = max(driver_income, CONFIG['MIN_FARE_THRESHOLD'])
                gap_subsidy = actual_income - driver_income
                subsidy_out = gap_subsidy + subsidy[loc]

                step_subsidy_cost += subsidy_out
                zone_profits[loc] += price * commission - subsidy_out
            ...
            for i in unmatched:
                rewards[d_list[i]] = CONFIG['IDLE_REWARD']
        ...
        avg_wait_time = self.total_wait_time / max(self.total_generated_orders, 1)
        avg_wait_time_served = self.served_wait_time / max(self.total_served_orders, 1)
        info = {
            ...
            'served_wait_time': self.served_wait_time,
            'avg_wait_time': avg_wait_time,
            'avg_wait_time_served': avg_wait_time_served,
            ...
            'driver_status': self.driver_status.copy(),
        }
    ...
    def _update_driver_online_status(self, surge, subsidy):
        for i in range(CONFIG['N_DRIVERS']):
            ...
            if self.driver_status[i] == 2:
                ...
                if self.driver_status[i] == 0 and self.driver_type[i] == 0 and self.driver_daily_income[i] >= DRIVER_CONFIG['PART_TIME_INCOME_TARGET']:
                    ...
                    if recent_income_rate < DRIVER_CONFIG.get('FULL_TIME_MIN_THRESHOLD', 0.1) and np.random.rand() < 0.2:
                        self.driver_status[i] = 2

    def get_proxy_state(self, driver_idx, dest_idx):
        if not hasattr(self, 'cached_grid_features'):
            return np.zeros(CONFIG['STATE_DIM'])
        spatial_features = [self.cached_grid_features[dest_idx]]
        for direct in range(6):
            n_idx = self.neighbor_matrix[dest_idx, direct]
            spatial_features.append(self.cached_grid_features[n_idx] if n_idx != -1 else np.full(6, -1.0))
        flat_spatial = np.concatenate(spatial_features)
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        print(f'step: {i}, reward: {reward.mean():.4f}, done: {done}, avg_wait: {info["avg_wait_time"]:.4f}')
